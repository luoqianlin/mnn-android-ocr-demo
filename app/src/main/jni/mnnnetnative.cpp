...
//    config.mean[3] = 127.5f;
//    config.normal[3] = 1 / 127.5f;
    // filterType、wrap
...
Java_com_clipper_ClipperLib_clipperOffset(JNIEnv *env, jclass clazz, jobjectArray pts,
                                          jdouble delta) {
    ClipperLib::Path subj;
    jsize length = env->GetArrayLength(pts);
    for (jsize i = 0; i < length; i++) {
        jfloatArray pJobject = (jfloatArray) (env->GetObjectArrayElement(pts, i));
        jfloat *points = env->GetFloatArrayElements(pJobject, 0);
        subj << ClipperLib::IntPoint(points[0], points[1]);
        env->ReleaseFloatArrayElements(pJobject, points, 0);
    }
    ...
    jobjectArray arrDim0 = env->NewObjectArray(solution.size(), env->FindClass("[[F"), nullptr);
    jclass dim_class1 = env->FindClass("[F");
    for (size_t i = 0; i < solution.size(); i++) {
        auto points = solution[i];
        jobjectArray arrDim1 = env->NewObjectArray(points.size(), dim_class1, nullptr);
        env->SetObjectArrayElement(arrDim0, i, arrDim1);
        for (size_t j = 0; j < points.size(); j++) {
            auto point = points[j];
            jfloatArray jPoints = env->NewFloatArray(2);
            jfloat buf[2];
            buf[0] = point.X;
            buf[1] = point.Y;
            env->SetFloatArrayRegion(jPoints, 0, 2, buf);
            env->SetObjectArrayElement(arrDim1,j,jPoints);
        }
    }
    return arrDim0;
}